...
def test_MChain_get_ess_partial_tracerer(thirty_taxa_MChain):
    ...
    for ess_key in thirty_taxa_MChain.get_key_names():
        result.append(thirty_taxa_MChain.get_ess(ess_key=ess_key, upper_i=200, lower_i=100))
...
def test_MChain_get_ess_partial_wronglowertype(thirty_taxa_MChain):
    with pytest.raises(ValueError):
        thirty_taxa_MChain.get_ess(ess_key="posterior", lower_i=10.2)


def test_MChain_get_ess_partial_wronguppertype(thirty_taxa_MChain):
    with pytest.raises(ValueError):
        thirty_taxa_MChain.get_ess(ess_key="posterior", upper_i=10.2)


def test_MChain_get_ess_partial_uppertoobig(thirty_taxa_MChain):
    with pytest.raises(IndexError):
        thirty_taxa_MChain.get_ess(ess_key="posterior", upper_i=3000)


def test_MChain_get_ess_partial_wrongway(thirty_taxa_MChain):
    with pytest.raises(ValueError):
        thirty_taxa_MChain.get_ess(ess_key="posterior", lower_i=10, upper_i=5)


def test_MChain_get_ess_partial_neglower(thirty_taxa_MChain):
    with pytest.raises(ValueError):
        thirty_taxa_MChain.get_ess(ess_key="posterior", lower_i=-10)


def test_MChain_get_ess_partial_negupper(thirty_taxa_MChain):
    with pytest.raises(ValueError):
        thirty_taxa_MChain.get_ess(ess_key="posterior", upper_i=-10)


def test_MChain_get_ess_partial_correctness(thirty_taxa_MChain):
    result = []
    for ess_key in thirty_taxa_MChain.get_key_names():
        result.append(thirty_taxa_MChain.get_ess(ess_key=ess_key))

    result_partial = []
    for ess_key in thirty_taxa_MChain.get_key_names():
        result_partial.append(thirty_taxa_MChain.get_ess(ess_key=ess_key, lower_i=0,
...
def test_MChain_spectral_clustree(thirty_taxa_MChain):
    for beta in [1, 2, 0.5]:
