...
Tree_List read_trees_from_file(char* filename){
    ...
    if ((f = fopen(filename, "r"))){
        long num_leaves;
        int num_trees;
        char * first_ints_buffer = malloc(10 * sizeof(char)); // max number of digits for number of leaves and number of trees in file is assumed to be 20
        // read in first two lines: number of leaves and number of trees
        if (fgets(first_ints_buffer, 10 * sizeof(char), f) != NULL){
            num_leaves = atoi(first_ints_buffer);
            if (num_leaves == 0){
        ...
        while(fgets(buffer, max_str_length * sizeof(char), f) != NULL){
            ...
            while((cluster_list = strsep(&partial_tree_str, "}")) != NULL){
                ...
                if(strlen(cluster_list) > 0){ //ignore last bit (just contained ])
                    printf("cluster list: %s\n", cluster_list);
                    // Find leaves in clusters
                    char * cluster;
                    while((cluster = strsep(&cluster_list, ",")) != NULL){
                        long actual_leaf = atol(cluster);
                        printf("actual leaf: %ld\n", actual_leaf);
                        // update node relations if current leaf appears for first time
                        printf("current tree: %ld, actual leaf: %ld ", current_tree, actual_leaf);
                        printf("parent: %ld\n", tree_list.trees[current_tree][actual_leaf - 1].parent);
...
Tree_List read_tree_from_string(int num_leaves, char* tree_string){
    ...
    Tree_List tree_list;
    tree_list.num_leaves = num_leaves;
    tree_list.num_trees = 1;
    tree_list.trees = malloc(sizeof(Node*));
    tree_list.trees[0] = malloc(num_nodes * sizeof(Node));
...
int nni_move(Node * tree, long rank_in_list, long num_leaves, int child_moves_up){
    if (tree == NULL){
...
int rank_move(Node * tree, long rank_in_list, long num_leaves){
    long num_nodes = 2 * num_leaves - 1;
    if (tree == NULL){
        ...
        if (tree[rank_in_list].parent == rank_in_list + 1){
            ...
            for (int i = 0; i < 2; i++){
                ...
                long upper_child = tree[rank_in_list + 1].children[i];
...
long mrca(Node * tree, long node1, long node2){
    long rank1 = node1;
    long rank2 = node2;
    while (rank1 != rank2){
        if (rank1 < rank2){
...
// returns a path in matrix representation -- explanation in data_structures.md
long findpath_distance(Node *start_tree, Node *dest_tree, long num_leaves){
    ...
    //     moves[i][1] = 0;
    // }
    long path_index = 1; // next position on path that we want to fill with a tree pointer
    if (start_tree == NULL){
        printf("Error. Start tree doesn't exist.\n");
    } else if (dest_tree == NULL){
        printf("Error. Destination tree doesn't exist.\n");
            ...
            while(current_mrca != i){
                ...
                for (int child_index = 0; child_index < 2; child_index++){ // find out if one of the children of current_tree[current_mrca] has rank current_mrca - 1. If this is the case, we want to make an NNI
                    if (did_nni == false && current_tree[current_mrca].children[child_index] == current_mrca - 1){ // do nni if current interval is an edge
                        ...
                        int child_stays; // index of the child of current_tree[current_mrca] that does not move up by an NNI move
                        // find the index of the child of the parent of the node we currently consider -- this will be the index child_stays that we want in the end
                        long current_child_index = dest_tree[i].children[0]; // rank of already existing cluster in both current_tree and dest_tree
                        while (found_child == false){
                            while (current_tree[current_child_index].parent < current_mrca - 1){ // find the x for which dest_tree[i].children[x] is contained in the cluster induced by current_tree[current_mrca - 1]
                                current_child_index = current_tree[current_child_index].parent;
                            }
...
int main(){
    ...
    long num_leaves = tree_list.num_leaves;
    long num_nodes = 2 * num_leaves - 1;
    ...
    long distance = findpath_distance(tree_list.trees[0], tree_list.trees[1], tree_list.num_leaves);
